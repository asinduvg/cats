...
import part2abstractMath.UsingMonads.{OptionHttpService, config}
...
object UsingMonads {
  ...
  val aChangedLoading = loadingMonad
    .flatMap(anEither)(n => if (n % 2 == 0) Right(n + 1) else Left("Loading meaning of life..."))
...
//  }
  ...
  /*
    Requirements:
      - if the host and port are found in the configuration map, then we'll return a connection with those values
