...
object Kleislis {
  ...
  type InterstingKleisli[A, B] = Kleisli[Id, A, B] // wrapper over A => Id[B]
  // InterstingKleisli = Reader!
  // hint
  ...
  val times2Reader = Reader[Int, Int](x => x * 2)
  val plus4Reader = Reader[Int, Int](y => y + 4)
  val composedReader = times2.flatMap(t2 => plus4.map(p4 => t2 + p4))
  val composedForReader = for {
    t2 <- times2
    p4 <- plus4
  } yield t2 + p4
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(composedForReader(3)) // 13
  }
